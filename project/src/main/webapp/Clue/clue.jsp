	...
	layui.use('table', function() {
		...
		table.render({
			...
			table.on('toolbar(test)', function(obj) {
				...
				}else if(obj.event === 'manychange') {
					var checkStatus = table.checkStatus('demo');
					if(checkStatus.data.length==0){
						parent.layer.msg('请先选择要转化的数据行！', {icon: 2});
						return ;
					}
					for(var i=0;i<checkStatus.data.length;i++){
						checkStatus.data[i].userid=${user.id};
					}
					parent.layer.msg('转化中...', {icon: 16,shade: 0.3,time:1000});
					 $.ajax({
	                        type:"POST",
	                        url: 'manychange',
	                        data:JSON.stringify(checkStatus.data),
	                        ...
	                        success:function (json) {
	                            layer.closeAll('loading');
	                            if(json==1){
	                                parent.layer.msg('转化成功！', {icon: 1,time:2000,shade:0.2});
	                                fresh('demo')
	                            }else{
	                                parent.layer.msg('转化失败！', {icon: 2,time:2000,shade:0.2});
	                                fresh('demo')
	                            }
	                        }
	                    })
